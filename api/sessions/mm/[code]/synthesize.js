...
function perQuestionPrompt(questionText, evalAnswers, selfAnswer) {
  ...
  const selfBlock = selfAnswer
    ? `\nThe subject's own answer to this question: "${selfAnswer}"\n(Include this in quotes if directly relevant. Do NOT let it skew the pattern analysis — patterns should reflect evaluator responses only.)\n`
    : '';
...
Return strict JSON:
{
  "patterns": ["1-3 sentence observations about themes appearing across multiple responses"],
...
- Write patterns/outliers/absences as full, standalone sentences (not fragments).
...
Below are the cross-question synthesis summaries. Do NOT simply summarize them — distill the 3-5 most important, cross-cutting things ${subjectName} needs to understand from this full picture.
...
{
  "themes": [
    "theme 1 — 2-3 sentences, direct, specific, humane",
    "theme 2",
    "theme 3"
  ]
}
...
- This is what they read if they read nothing else. Make it honest, direct, and worth reading.
- Avoid generic sentences. Every theme should be specific to what the evaluators said.
- Output ONLY the JSON. No preamble, no markdown fences.`;
}
...
What evaluators said about ${subjectName} (key themes):
- ${themeSummary}

What ${subjectName} said about themselves (self-survey answers):
${selfBlock}
...
- Gaps can be positive (evaluators saw more than ${subjectName} sees in themselves) or developmental.
  ...
  try {
    ...
    if (completedEvals.length < MIN_RESPONSES) {
      return res.status(400).json({
        error: `Need at least ${MIN_RESPONSES} evaluators to have responded (${completedEvals.length} so far)`,
      });
    }
    ...
    const responses = responseRaws.map((r, i) => {
      if (!r) return null;
      try { return typeof r === 'string' ? JSON.parse(r) : r; } catch { return null; }
    }).filter(Boolean);

    // Load self-survey (optional)
    const selfRaw = await redis.get(`mm:${uc}:self`);
    const selfRecord = selfRaw
      ? (typeof selfRaw === 'string' ? JSON.parse(selfRaw) : selfRaw)
      : null;
    const selfAnswers = selfRecord?.answers || {};

    const { questions, questionOrder } = config;

    // ── Step 1: 6 parallel per-question synthesis calls ──────────────────────
    const perQuestionResults = await Promise.all(
      questionOrder.map(async (qid) => {
        const q = questions.find(q => q.id === qid);
        if (!q) return null;

        const evalAnswers = responses
          .map(r => (r.answers?.[qid] || '').trim())
          .filter(Boolean);

        if (evalAnswers.length === 0) {
          return { questionId: qid, patterns: [], outliers: [], absences: [], quotes: [], categoryNote: '' };
        }

        const result = await callClaude(
          perQuestionPrompt(q.text, evalAnswers, selfAnswers[qid] || null),
          2000,
        );
        return {
          questionId: qid,
    ...
    // ── Step 2: Overview synthesis ────────────────────────────────────────────
    const overviewResult = await callClaude(
      overviewPrompt(config.subjectName, perQuestionResults.filter(Boolean)),
      1500,
    );
    const overview = { themes: overviewResult.themes || [] };

    // ── Step 3: Self-reflection (if self-survey exists) ───────────────────────
    let selfReflection = null;
    if (selfRecord && Object.values(selfAnswers).some(v => v?.trim())) {
      const srResult = await callClaude(
        selfReflectionPrompt(
          config.subjectName,
          overview.themes,
          perQuestionResults.filter(Boolean),
          selfAnswers,
          questionOrder,
          questions,
        ),
      ...
      selfReflection = {
        alignments: srResult.alignments || [],
        gaps: srResult.gaps || [],
      };
    }
    ...
    const report = {
      ...
      overview,
      perQuestion: perQuestionResults.filter(Boolean),
      selfReflection,
    };
