      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
        {THINK_PIECES.map((piece) => (
          <a key={piece.id} onClick={(e) => { e.preventDefault(); navigate(piece.id); }} href="#"
             style={{ display: 'block', background: C.bgCard, borderRadius: 4, padding: '36px 36px', textDecoration: 'none', color: 'inherit', borderLeft: `3px solid ${C.sage}`, transition: 'background 0.3s, transform 0.3s', cursor: 'pointer' }}
            ...
            <p style={{ fontFamily: F.serif, fontStyle: 'italic', fontSize: 18, color: C.sage, marginBottom: 16, lineHeight: 1.4 }}>{piece.subtitle}</p>
            <p style={{ fontSize: 15, color: C.creamMuted, lineHeight: 1.7, marginBottom: 16 }}>{piece.abstract}</p>
            <span style={{ display: 'inline-flex', alignItems: 'center', gap: 8, color: C.sage, fontFamily: F.sans, fontSize: 12, letterSpacing: '0.18em', textTransform: 'uppercase' }}>
              Read <span>→</span>
            </span>
          </a>
        ))}
